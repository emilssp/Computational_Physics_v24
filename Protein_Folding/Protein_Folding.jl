...
# ╔═╡ ff389f73-f0ec-466f-ad23-3a619ed63573
begin
	...
	function randomStep()
		non_zero_position = rand(1:2)
		non_zero_value = rand([-1, 1])
		step = zeros(Int, 2)
		step[non_zero_position] = non_zero_value
	return step
	end
...
# ╔═╡ 8f63abb9-c8b5-4395-8e61-6037750bcf4d
	heatmap(interaction_mat, color =:jet, title="Monomer-Monomer Interaction")

# ╔═╡ c9168737-2cc9-4013-b8cd-6ba1263a36c1
begin
	mutable struct Monomer
		kind::Aminoacid #type of the monomer
		nearest_neighbors::Vector{Monomer}
		pos::Vector{Int64}
		function Monomer(kind::Aminoacid ,pos::Vector{Int64})
			new(kind,Vector{Monomer}(),pos)
		end
	end
	function addNeighbors(monomer::Monomer, monomers::Vector{Monomer})
		other_monomers = filter(m -> m !== monomer, monomers)
		for om ∈ other_monomers
			if norm(monomer.pos-om.pos) == 1
				push!(monomer.nearest_neighbors, om)
			end
		end
	end
end

# ╔═╡ b3dddbfd-3671-4929-8e82-eac203da2a15
begin
	function randomMonomers(N::Int64)
		aminos = [Aminoacid(rand(1:20)) for _ ∈ 1:N]
		monomers = [Monomer(aminos[1],[2,2])]
		i = 2
		while i<=N 
			step = randomStep()
			new_pos = monomers[i-1].pos + step
			if all(m.pos != new_pos for m ∈ monomers)
				push!(monomers,Monomer(aminos[i],new_pos))
				i+=1
			end
		end
		return monomers
	...
	function straightPolymer(N::Int64)
...
# ╔═╡ 1aecb81f-cabb-48d7-b189-81f870fab2bd


# ╔═╡ 7a809c38-de52-4136-96df-c418bd04a8b7
struct Polymer
	N::Int64 #number of monomers in the structure
	monomers::Vector{Monomer} #constituents of the polymer
	energy::Float64
	function Polymer(monomers::Vector{Monomer}, T::Float64)
		N = length(monomers)
		energy = 0.00
		for m ∈ monomers
			addNeighbors(m, monomers)
		...
		energy = energy*T#/2 should I divide by two to account for double counting of the energy
...
begin
		pm = Polymer(randomMonomers(15), 10.00)
		print(pm.energy)
end
...
begin
	...
	scatter!(x_positions, y_positions, 
			...
			title="Polymer Positions", color=:green,
			markersize=5, legend=false)
end

# ╔═╡ b3248853-3c74-4793-9079-9c5f51e79931
